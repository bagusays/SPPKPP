...
<div>
    ...
    <div class="content-wrapper">
        <div class="container-fluid">
            <ol class="breadcrumb">
                <li style="font-size: 15px;" class="breadcrumb-item"> <router-link to="/kriteria/deadlinehari">List Kriteria Deadline Hari</router-link>
                </li>
                <li class="breadcrumb-item active">
                    Edit Kriteria Deadline Hari
                </li>
            </ol>
            <div class="card">
                <div class="card-header"><span>Edit Kriteria Deadline Hari</span></div>
                <div class="card-body">
    ...
    methods: {
        ...
        async getDetail() {
            try {
                ...
                let result = await this.$helpers.axiosAuth.post(`/kriteria/deadlinehari/detail`, param)
        ...
        async onSubmit() {
            ...
            if(validate) {
                let res = await this.$helpers.axiosAuth.post(`/kriteria/deadlinehari/edit`, this.form)
                if(res.status == 200) {
                    await this.$helpers.alert.success(res.data.message)
                    this.$router.push({ path: '/kriteria/deadlinehari' })
                } else
