...
<div>
    ...
    <div class="content-wrapper">
        <div class="container-fluid">
            <ol class="breadcrumb">
                <li style="font-size: 15px;" class="breadcrumb-item"> <router-link to="/kriteria/jarakpengiriman">List Kriteria Jarak Pengiriman</router-link>
                </li>
                <li class="breadcrumb-item active">
                    Edit Kriteria Jarak Pengiriman
                </li>
            </ol>
            <div class="card">
                <div class="card-header"><span>Edit Kriteria Jarak Pengiriman</span></div>
                <div class="card-body">
    ...
    methods: {
        ...
        async getDetail() {
            try {
                ...
                let result = await this.$helpers.axiosAuth.post(`/kriteria/jarakpengiriman/detail`, param)
        ...
        async onSubmit() {
            ...
            if(validate) {
                let res = await this.$helpers.axiosAuth.post(`/kriteria/jarakpengiriman/edit`, this.form)
                if(res.status == 200) {
                    await this.$helpers.alert.success(res.data.message)
                    this.$router.push({ path: '/kriteria/jarakpengiriman' })
                } else
